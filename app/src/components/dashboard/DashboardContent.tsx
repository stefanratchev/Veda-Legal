...
function StatCard({ icon, value, label, trend, variant, delay = 0 }: StatCardProps) {
  const variantStyles = {
    ...
    hours: "bg-[var(--warning-bg)] text-[var(--accent-pink)]",
  };

  return (
    <div
      className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded p-4
                 hover:border-[var(--border-accent)] hover:-translate-y-0.5 hover:shadow-lg
                 transition-all duration-300 animate-fade-up"
      style={{ animationDelay: `${delay}s` }}
    >
      <div className="flex items-start justify-between mb-3">
        <div className={`w-10 h-10 rounded flex items-center justify-center ${variantStyles[variant]}`}>
          {icon}
        </div>
        {trend && (
          <span
      ...
      <div className="font-heading text-2xl font-semibold text-[var(--text-primary)] mb-0.5">
        {value}
      </div>
...
function QuickAction({ icon, label, onClick }: QuickActionProps) {
  return (
    <button
      onClick={onClick}
      className="
        relative p-3.5 rounded border border-[var(--border-subtle)] bg-[var(--bg-surface)]
        text-center transition-all duration-200
        hover:border-[var(--border-accent)] hover:-translate-y-0.5
        before:absolute before:inset-0 before:rounded
        before:bg-gradient-to-br before:from-[var(--accent-pink-glow)] before:to-transparent
        before:opacity-0 hover:before:opacity-100 before:transition-opacity
      "
    >
      <div className="relative z-10">
        <div className="w-8 h-8 mx-auto mb-2 rounded bg-[var(--bg-hover)] flex items-center justify-center text-[var(--accent-pink)]">
          {icon}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-[1.6fr_1fr] gap-4">
        ...
        <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded overflow-hidden animate-fade-up delay-3">
          <div className="px-4 py-3 border-b border-[var(--border-subtle)] flex items-center justify-between">
            <h3 className="font-heading text-base font-semibold text-[var(--text-primary)]">
              Recent Clients
            </h3>
            <a href="/clients" className="text-[13px] text-[var(--accent-pink)] hover:gap-2 flex items-center gap-1 transition-all">
              View All
