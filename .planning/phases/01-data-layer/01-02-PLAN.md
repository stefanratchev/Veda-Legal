...
<task type="auto">
  <name>Task 1: Update ReportsContent TypeScript interfaces</name>
...
<task type="auto">
  <name>Task 2: Update server component getReportData() to match API route</name>
  <files>app/src/app/(authenticated)/(admin)/reports/page.tsx</files>
  <action>
Mirror the changes from Plan 01's route.ts into page.tsx's `getReportData()` function. The two code paths must produce identical response shapes. Task 1 has already updated the TypeScript interfaces, so this code will type-check immediately.
